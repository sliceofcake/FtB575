...
function MU_processSub($row,&$in){
	...
	*/
	...
	foreach ($_FILES as $officialname=>$fileE){
		$prefixS = $row["req"]."_";
		if (mb_substr($officialname,0,mb_strlen($prefixS)) === $prefixS){
...
function MU_fxn(&$p,&$o,&$in){global $MU_TABLE;
	$d = &$p["dat"];
	$f = &$p["flO"];
	if (in_array($p["act"],["get","new","edt","del"],T)){
		...
		switch ($p["act"]){default:;
			...
			break;case "new":
				...
				foreach ($f as $filename=>$fileE){
					...
					if (!is_boring($p["tbl"]) || !is_boring($filename) || !in_array($extension,FILE_EXTENSION_WHITELIST(),T) || !isI($insertID)){SET_RETURN_MSG("ERROR : INVALID FILEPATH COMPONENT [ESOTERIC]");return F;}
					move_uploaded_file($fileE["tmp_name"],$GLOBALS["ROOT"]["DIR_FILE_CHART"].$p["tbl"]."/".$filename."_".str($insertID).$extension);}
				//....
				switch ($p["tbl"]){default:;
					break;case "chart"       :;
					break;case "user"        :;
			...
			break;case "edt":
				...
				foreach ($f as $filename=>$fileE){
					// these parts are clean, but let's make sure
					$extension = π_calcFileExtensionProvided($fileE["name"]);
					if (!is_boring($p["tbl"]) || !is_boring($filename) || !in_array($extension,FILE_EXTENSION_WHITELIST(),T)){SET_RETURN_MSG("ERROR : INVALID FILEPATH COMPONENT [ESOTERIC]");return F;}
					move_uploaded_file($fileE["tmp_name"],$GLOBALS["ROOT"]["DIR_FILE_CHART"].$p["tbl"]."/".$filename.$extension);}
				//....
				switch ($p["tbl"]){default:;
					break;case "chart"       :;
					break;case "user"        :;
...
$MU_TABLE["n/a"]["FtB575_1|plushie_gen"] = function(&$p,&$d,&$o){
	...
	if ($successF1){
		...
		if (!pVerify($pCopy1["dat"]["passwordS"],$row["hash"])){SET_RETURN_MSG("ERROR : BAD AUTHENTICATION PASSWORD");return F;}
		$ID = $pCopy1["dat"]["ID"];}
